...
$.ajax({
    ...
    success: function (response) {
        ...
        Object.entries(response).forEach(([date, count]) => {
            massiveData.push({ date: date.substring(0, 10), value: count });
        });
...
(function($) {
    $.fn.glanceyear = function(massive, options) {
        ...
        for (var i=0; i<54; i++) {
            ...
            firstDate.setDate(settings.today.getDate() - dayCount);
        ...
        function fillData(massive) {
           ...
           var s= massive[m].date;
           var e= massive[m].value;
                var dateComponents = s.split("-");
                var month = parseInt(dateComponents[1], 10).toString();
                var day = parseInt(dateComponents[2], 10).toString();
                var formattedDate = dateComponents[0] + "-" + month + "-" + day;

                console.log(formattedDate);
               $_this.find('rect.day[data-date="' + formattedDate + '"]').attr('data-count', massive[m].value);
